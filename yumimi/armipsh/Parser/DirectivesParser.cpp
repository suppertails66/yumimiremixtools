...
std::unique_ptr<CAssemblerCommand> parseDirectiveShArch(Parser& parser, int flags)
{
	...
	switch (flags)
	{
	case DIRECTIVE_SH_SH1:
		Sh.setVersion(SARCH_SH1);
		return make_unique<ArchitectureCommand>(L".sh1", L".sh1");
	case DIRECTIVE_SH_SH2:
		Sh.setVersion(SARCH_SH2);
		return make_unique<ArchitectureCommand>(L".sh2", L".sh2");
	}
...
const DirectiveMap directives = {
	...
	{ L".sh1",				{ &parseDirectiveShArch,			DIRECTIVE_SH_SH1 } },
	{ L".sh2",				{ &parseDirectiveShArch,			DIRECTIVE_SH_SH2 } },
